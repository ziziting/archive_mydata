...
class DataPtArchive:
    ...
    def Data_Purge(self):
        pt_cmd = f"""{ARCHIVE_TOOL} --source h={self.source_host},P={self.source_port},u={DUMP_USER},p={DUMP_PWD},D={self.db_name},t={self.tb_name},i={self.ix_name},A=utf8mb4   --progress {self.tnx} --sleep 1  --where "{self.condition_sql}" --statistics  --limit={self.tnx} --txn-size {self.tnx} --max-lag=5 --retries 5 --purge """
        exec_code, output = subprocess.getstatusoutput(pt_cmd)
        return exec_code, output
